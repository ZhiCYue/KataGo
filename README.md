...
- **提示讲解**：点「提示」后，棋盘标出前三候选点（①首选／②③次选），并给出规则化讲解——目标、急所推荐（据候选胜率差判断是否唯一手）、当前死活、思路口诀。
- **形势热力图**：侧栏「形势」开关（点「提示」会自动点亮），按 KataGo 归属给交叉点着色（青黛属黑、朱红属白），一眼看清谁死谁活；随每手应手实时刷新。开关是**粘性**的——打开后跨重做／换题一直保留，直到再次点「形势」才关闭。
...
- `GET  /api/tsumego-search?q=关键词` —— 联网搜索公开死活题资源链接（保留的后端接口，页面入口已移除）
...
- [x] 形势热力图（死活模式已实现：ownership 着色 + 候选点 + 讲解）
- [ ] 复盘 / SGF 记谱与导出
- [ ] 让子棋、计时
